...
$B.$isNode = function(obj){
    for(var i=0, len = $DOMNodeAttrs.length; i < len;i++){
...
$B.$isEvent = function(obj){
    flag = true
    for(var i=0, len = $DOMEventAttrs_W3C.length; i < len;i++){
        if(obj[$DOMEventAttrs_W3C[i]]===undefined){flag=false;break}
    }
    if(flag) return true
    for(var i=0, len = $DOMEventAttrs_IE.length; i < len;i++){
...
DOMNode.__add__ = function(self,other){
    ...
    if(isinstance(other,$TagSum)){
        for(var $i=0, len = other.children.length; $i < len;$i++){res.children.push(other.children[$i])}
    ...
    if(self.elt.nodeType===9){ // Document
        if(typeof key==="string"){
            ...
            try{
                ...
                for(var $i=0, len = elts.length; $i < len;$i++) res.push($DOMNode(elts[$i]))
                return res
            }catch(err){
...
DOMNode.__le__ = function(self,other){
    ...
    if(isinstance(other,$TagSum)){
        var $i=0
        for($i=0, len = other.children.length; $i < len;$i++){
...
DOMNode.clone = function(self){
    ...
    if(ix_elt!=-1){
        ...
        for(var i=0, len = events.$keys.length; i < len;i++){
            var event = events.$keys[i]
            for(var j=0, len = events.$values[i].length; j < len;j++){
...
DOMNode.get = function(self){
    ...
    if($dict['name']!==undefined){
        ...
        for(var i=0, len = node_list.length; i < len;i++) res.push($DOMNode(node_list[i]))
    }
    ...
    if($dict['selector']!==undefined){
        ...
        var sel_res = []
        if(node_list.length===0) return []
        for(var i=0, len = node_list.length; i < len;i++) sel_res.push($DOMNode(node_list[i]))
        ...
        for(var i=0, len = res.length; i < len;i++){
            ...
            flag = false
            for(var j=0, len = sel_res.length; j < len;j++){
...
DOMNode.set_style = function(self,style){ // style is a dict
    for(var i=0, len = style.$keys.length; i < len;i++){
