var $module=(function($B){
    ...
    obj._is_valid = function(pattern) {
        ...
        var re_list=['\{,\d+\}']
        for(var i=0, len=re_list.length; i < len; i++) {
    ...
    obj.finditer = function(pattern,string,flags){
        ...
        var _list=[]
        for (var j=0, len=jsmatch.length; j < len; j++) {
            var mo = {}
            mo._match=jsmatch[j]
            mo.group = function(){
               var res = []
            ...
            mo.groups = function(_default){
               ...
               var res = []
               for(var i=1, len=jsmatch.length;i<len;i++){
    ...
    obj.sub = function(pattern,repl,string){
        ...
        if(typeof repl==="string"){
            ...
            var $repl1 = function(){
                ...
                groups = []
                for(var i=1, len=arguments.length-2;i<len;i++){groups.push(arguments[i])}
                mo.groups = function(_default){
                    if(_default===undefined){_default=None}
                    var res = []
