...
self.addEventListener('message', function(e) {
   ...
   importScripts('/src/brython_builtins.js', '/src/version_info.js',
    ...
    import_module_search_path = function(module,origin){
      ...
      for(var j=0; j < search.length; j++) {
        ...
        for(var i=0;i<path_list.length;i++){
           ...
           try {
               ...
               var module_contents=__BRYTHON__.$download_module(module.name, path+'.py')
               return [path+'.py', process_py_module(module,path,module_contents)]
      ...
      if(!flag){
        throw __BRYTHON__.builtins.ImportError("module "+module.name+" not found")
      }
    }
    ...
    self.postMessage({jscode:jscode, filepath:filepath})
