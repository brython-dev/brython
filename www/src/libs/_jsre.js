...
MatchObject_funcs.item = function(self, rank) {
...
MatchObject_funcs.group = function(self) {
    ...
    for (var i = 0, _len_i = arguments.length; i < _len_i; i++) {
        if (self.jsmatch[arguments[i]] === undefined) {res.push(_b_.None)} else {res.push(self.jsmatch[arguments[i]])}
    }
    if (arguments.length == 1) {return res[0]}
    return _b_.tuple.$factory(res)
}

MatchObject_funcs.groups = function(self, _default) {
    if (_default === undefined) {_default = _b_.None}
    var res = []
    for (var i = 1, _len_i = self.length; i < _len_i; i++) {
        if (self.jsmatch[i] === undefined) {res.push(_default)} else {res.push(self.jsmatch[i])}
    }
    return _b_.tuple.$factory(res)
}
MatchObject_funcs.start_get = function(self) {
    return self.index
}

MatchObject_funcs.start_set = _b_.None

MatchObject_funcs.end_get = function(self) {
    return self.length - self.index
}

MatchObject_funcs.end_set = _b_.None

MatchObject.tp_methods = ["item", "group", "groups"]

MatchObject.tp_getset = ["start", "end"]
...
var SRE_PatternDict = $B.make_type('SRE_Pattern')

var SRE_PatternDict_funcs = SRE_PatternDict.tp_funcs = {}

SRE_PatternDict_funcs.findall = function(self, string) {
    return obj.findall(self.pattern, string, self.flags)
}

SRE_PatternDict_funcs.finditer = function(self, string) {
    return obj.finditer(self.pattern, string, self.flags)
}

SRE_PatternDict_funcs.match = function(self, string) {
    return obj.match(self.pattern, string, self.flags)
}

SRE_PatternDict_funcs.search = function(self, string) {
    return obj.search(self.pattern, string, self.flags)
}

SRE_PatternDict_funcs.sub = function(self, repl, string) {
    return obj.sub(self.pattern, repl, string, self.flags)
}

SRE_PatternDict.tp_methods = ["findall", "finditer", "match", "search", "sub"]

$B.set_func_names(SRE_PatternDict, "_jsre")
$B.finalize_type(SRE_PatternDict)
...
obj.finditer = function(pattern, string, flags) {
    ...
    for (var j = 0, _len_j = jsmatch.length; j < _len_j; j++) {
        ...
        mo.group = function() {
           var res = []
           for (var i=0, _len_i = arguments.length; i < _len_i;i++) {
               if (jsmatch[arguments[i]] === undefined) {res.push(_b_.None)} else {res.push(jsmatch[arguments[i]])}
           }
           if (arguments.length == 1) {return res[0]}
           return _b_.tuple.$factory(res)
        }
        mo.groups = function(_default) {
           ...
           for (var i = 1, _len_i = jsmatch.length; i < _len_i; i++) {
              if (jsmatch[i] === undefined) {res.push(_default)} else {res.push(jsmatch[i])}
           }
           return _b_.tuple.$factory(res)
        }
...
obj.search = function(pattern, string) {
    var $ = $B.args('re.search', 2, {pattern:null, string:null}, 
                  arguments, null, 'args', 'kw')
    var args = $.args
    if (args.length>0) {var flags = args[0]} else {var flags = $B.$getattr($.kw, 'get')('flags', '')}
    flags = normflags(flags)
    var jsp = new RegExp(pattern, flags)
...
obj.sub = function(pattern, repl, string) {
    ...
    var args = $ns['args']
    var count = $B.str_dict_get($ns['kw'], 'count', 0)
    var flags = $B.str_dict_get($ns['kw'], 'flags', '')
    if (args.length > 0) {
        var count = args[0]
    }
    if (args.length > 1) {
        ...
        var $repl1 = function() {
            ...
            mo.groups = function(_default) {
                ...
                for (var i = 0, _len_i = groups.length; i < _len_i; i++) {
                    if (groups[i] === undefined) {res.push(_default)} else {res.push(groups[i])}
                }
                return res
            }
            mo.group = function(i) {
    ...
    if (count == 0) {flags += 'g'}
    var jsp = new RegExp(pattern, flags)
    if (typeof repl == 'function') {return string.replace(jsp, $repl1)} else {return string.replace(jsp, repl)}
}
