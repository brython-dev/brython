...
$FuncArgs.prototype.ast = function(){
    ...
    for(var arg of this.tree){
        if(arg.type == 'end_positional'){
            ...
            if(arg.op == '*' && arg.name != '*'){
                ...
                args.vararg.lineno = lineno
            }else if(arg.op == '**'){
                args.kwarg = new ast.arg(arg.name)
                args.kwarg.lineno = lineno
            }
        }else{
            default_value = false
            if(arg.has_default){
    ...
    var ast_obj = new ast.arguments(args.posonlyargs, args.args, args.vararg,
...
$FuncArgs.prototype.transition = function(token, value){
    ...
    function check(){
        ...
        for(var arg of context.tree){
            if(arg instanceof $FuncArgIdCtx){
                if(names.indexOf(arg.name) > -1){
                    $_SyntaxError(context,
