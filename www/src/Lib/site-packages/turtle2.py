...
class Screen(metaclass=Singleton):
    ...
    def _drawline(self, _turtle, coordlist=None,
        ...
        _an1 = svg.animate(Id="animateLine%s" % self.item_drawn_index,
        ...
        if self.item_drawn_index == 1:
            ...
            begin = "animateLine%s.end" % (self.item_drawn_index-1)
        _an1.setAttribute('begin', begin)
...
class Turtle(TPen, TNavigator):
    ...
    def __init__(self, shape=_CFG["shape"], visible=_CFG["visible"]):
        ...
        self.screen = Screen()
        self.name = shape
        self.svg, self.index = self.screen.create_svg_turtle(self, name=shape)
    ...
    def _goto(self, end):
        ...
        if self._speed is not None:
            begin, duration, x, y = self.screen._drawline(self,
                                  (self._position, end),
                                  self.color(), self._pensize,
                                  self._speed)
        x0, y0 = self.screen._convert_coordinates(self._position[0], self._position[1])

        self.svg <= svg.set(attributeName="display", attributeType="CSS",
                            to="block",
                            begin=begin, dur="0s",
                            fill="remove")

        self.svg <= svg.animateMotion(From="%s,%s" % (x0, y0),
                                      to="%s,%s" % (x, y),
                                      dur=duration, begin=begin)

        self.svg <= svg.set(attributeName="display", attributeType="CSS",
                            to="block",
                            begin=begin, dur=duration,
                            fill="freeze")
