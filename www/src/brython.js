...
this.env=[]
if(this.name=='FF'){
var h=' '.repeat(node.indent+12),h8=' '.repeat(node.indent+8),h16=' '.repeat(node.indent+16)
...
if(this.star_arg){js +='$locals["'+this.star_arg+'"]=pos_args.slice(i)'
...
if(this.kw_arg){js +='{$locals["'+this.kw_arg+'"].$string_dict[attr] = kw_args[attr]}'
...
$ObjectDict.__getattribute__=function(obj,attr){if(attr=='__doc__')return _b_.None
...
for(var j=0,_len_j=items.length;j < _len_j;j++){upargs[pos++]={$nat:"kw",name:items[j][0],value:items[j][1]}}
break
default:
upargs[pos++]=$arg
}
...
}else if($other_kw!=null){$dict_keys[key_pos++]=$arg.name
...
&& typeof value=='function'){obj.$dict.$methods[attr]=$B.make_method(attr,obj.$dict,value,value)
return
}
var res=obj[attr]
...
if(self.$infos && self.$infos[attr]){if(attr=='__code__'){var res={__class__:$B.$CodeDict}
...
$DictDict.clear(res)
var si=$DictDict.__setitem__
for(var attr in obj){if(attr.charAt(0)!='$')si(res,attr,obj[attr])
}
return res
}
...
}else if(isinstance(arg,DOMNode)){args[pos++]=arg.elt
...
DOMNode.children=function(self){var res=[],pos=0
