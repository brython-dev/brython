...
$B.last(C.tree)+eval(value)}else{C.tree.push(value)}
return C
case 'JoinedStr':
if(C.tree.length > 0 &&
...
slots[key1]=value}}}
var missing=[]
for(var attr in slots){if(slots[attr]===null){if($dobj[attr]!==undefined){slots[attr]=$dobj[attr]}else{missing.push("'"+$B.from_alias(attr)+"'")}}}
