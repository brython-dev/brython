...
# issue 2926 - io.StringIO / io.BytesIO 'closed' attribute
import io

sio_2926 = io.StringIO("foo")
assert sio_2926.closed is False
assert sio_2926.line_buffering is False
assert sio_2926.newlines is None
sio_2926.close()
assert sio_2926.closed is True
assert_raises(ValueError, sio_2926.write, "bar")
# the other stream properties report the closed stream rather than a value
assert_raises(ValueError, lambda: sio_2926.newlines)
assert_raises(ValueError, lambda: sio_2926.line_buffering)

bio_2926 = io.BytesIO(b"foo")
assert bio_2926.closed is False
bio_2926.close()
assert bio_2926.closed is True
assert_raises(ValueError, bio_2926.read)
assert_raises(ValueError, bio_2926.write, b"bar")

with io.BytesIO() as ctx_2926:
    assert ctx_2926.closed is False
assert ctx_2926.closed is True

with io.StringIO() as sctx_2926:
    assert sctx_2926.closed is False
assert sctx_2926.closed is True
...
class ClosingIO_2926(io.IOBase):
    def close(self):
        return True

assert ClosingIO_2926().__exit__(None, None, None) is True

suppressed_2926 = True
try:
    with ClosingIO_2926():
        raise ValueError("boom")
except ValueError:
    suppressed_2926 = False
assert suppressed_2926

class QuietIO_2926(io.IOBase):
    def close(self):
        return None

assert QuietIO_2926().__exit__(None, None, None) is None
raised_2926 = False
try:
    with QuietIO_2926():
        raise ValueError("boom")
