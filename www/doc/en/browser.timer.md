...
`set_timeout(`_function, ms, *args_`)`
> runs `function(*args)` after *ms* milliseconds. Returns an object usable in
> the function `clear_timeout()` documented below
...
`set_interval(`_function, ms, *args_`)`
> launches repeated execution of `function(*args)` every *ms* milliseconds and
