...
trans_menu = {
    'menu_console': {'en': 'Console', 'es': 'Consola',
                     'fr': 'Console', 'pt': 'Console'},

    'menu_editor': {'en': 'Editor', 'es': 'Editor',
                    'fr': 'Editeur', 'pt': 'Editor'},

    'menu_gallery': {'en': 'Gallery', 'es': 'Galería',
                     'fr': 'Galerie', 'pt': 'Galeria'},

    'menu_doc': {'en': 'Documentation', 'es': 'Documentación',
                 'fr': 'Documentation', 'pt': 'Documentação'},

    'menu_download': {'en': 'Download', 'es': 'Descargas',
                      'fr': 'Téléchargement', 'pt': 'Download'},

    'menu_dev': {'en': 'Development', 'es': 'Desarrollo',
                 'fr': 'Développement', 'pt': 'Desenvolvimento'},

    'menu_groups': {'en': 'Groups', 'es': 'Grupos',
                    'fr': 'Groupes', 'pt': 'Grupos'}
}
links = {
    'home': 'index.html',
...
def show(prefix=''):
    # detect language
    language, has_req, qs_lang = "en", False, doc.query.getfirst("lang")
    if qs_lang and qs_lang in ("en", "fr", "es", "pt"):
        has_req, language = True, qs_lang
    else:
        import locale
        try:
    ...
    for key in ('home', 'console', 'editor', 'gallery',
        ...
        if has_req and key not in ('download', 'dev'):
            href += '?lang=%s' % language  # add lang to href
        if key == 'home':
